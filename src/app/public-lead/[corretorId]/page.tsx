...
export default function CapturaLeadPage({ params }: { params: Promise<{ corretorId: string }> }) {
  ...
  useEffect(() => {
    const fetchCorretorInfo = async () => {
      try {
        ...
        const corretorId = resolvedParams.corretorId;
        setFormData(prev => ({ ...prev, corretorId }));
        const info = await getCorretorInfo(corretorId);
    ...
    if (isSuccess) {
      const timer = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            clearInterval(timer);
            // Redireciona para a página anterior ou recarrega
            window.history.back();
            return 0;
          }
          return prev - 1;
